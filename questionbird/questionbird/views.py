...
def questions_teacher(request):
	if request.method == "GET":
		if request.session.has_key('teacherid'):
			...
			unChineseQuestion = Question.objects.filter(category__contains="语文",question_state='待解决',question_changing=0)
			unMathQuestion = Question.objects.filter(category__contains="数学",question_state='待解决',question_changing=0)
			unEnglishQuestion = Question.objects.filter(category__contains="英语",question_state='待解决',question_changing=0)
...
def checkQuestion(request):
	if request.method == 'POST':
		...
		if question.question_changing == 0:
...
def questions_unEvaluate(request):
    if request.method =='GET':
        if request.session.has_key('teacherid'):
            teacherid = request.session['teacherid']
            teacherlist = Teacher.objects.filter(id=teacherid)
            unEvaluate = Question.objects.filter(solver_name=teacherid,answer_satis=0)
        ...
        if not request.POST.has_key('voice'):
            ...
            filename = filename+file.name[file.name.find('.'):]
            question.answer_voice = '\\site_media'+'\\files\\'+filename
            filename = os.getcwd()+'\\static'+'\\files\\'+filename
            destinaton = open(filename,'wb')
            for chunk in file.chunks():
                destinaton.write(chunk)
            destinaton.close()
